...
namespace Invector
{
    public abstract class vThirdPersonMotor : vCharacter
    {
        ...
        [SerializeField]
        protected float groundMinDistance = 0.1f;
        [SerializeField]
        ...
        [Tooltip("Lock all the player's input and movement")]
        [SerializeField]
        ...
        [SerializeField]
        protected float strafeSpeed = 1.5f;
        [Tooltip("Add extra speed for the strafe movement, keep this value at 0 if you want to use only root motion speed.")]
        [SerializeField]
        protected float crouchStrafeSpeed = 1.5f;
        ...
        [HideInInspector]
        public v3rdPersonCamera tpCamera;                   // acess camera info        
        [HideInInspector]
        ...
        [HideInInspector]
        public Quaternion freeRotation;                     // generic variables to find the correct direction         
        [HideInInspector]
        public float offSetPivot;                           // create a offSet base on the character hips         
        [HideInInspector]
        ...
        protected CombatID combatID = new CombatID();               // get the ID information from the current weapon
        protected GameObject currentCollectable;                    // information of any collectable item
        protected v_AIController targetEnemy;                       // enemy AI info     
        [HideInInspector]
        public vHUDController hud;                                  // acess hud controller 
        [HideInInspector]
        public vActionsController actionsController;                // acess action controller information
        [HideInInspector]
        public vMeleeManager meleeManager;                          // acess melee weapon information        
        [HideInInspector]
        public vTriggerDragable draggableBox;                       // access drag box        
        [HideInInspector]
        public Rigidbody _rigidbody;                                // access the rigidbody component
        [HideInInspector]
        public PhysicMaterial frictionPhysics, slippyPhysics;       // create physics material for the rigidboby
        [HideInInspector]
        public CapsuleCollider _capsuleCollider;                    // get capsule collider information                
        ...
        {
            if (lockMovement || inAttack) return;
        ...
        void StrafeMovement()
        {
            ...
            if (direction >= 0.7 || direction <= -0.7 || speed <= 0.1) canSprint = false;
        }
        ...
        protected bool jumpFwdCondition
        {
            get
            {
                ...
                return !Physics.CapsuleCast(p1, p2, _capsuleCollider.radius * 0.9f, transform.forward, out hit, 0.3f, groundLayer);
            }
        }
        ...
        {
            if (strafing && !actions && !lockMovement)
            {
                ...
                else //if(input != Vector2.zero)
                {
                    if (!inAttack)
                    {
                        ...
                        var eulerY = transform.eulerAngles.y;
                        if (diferenceRotation < -2.5 && canMoveLeft) eulerY = cameraTransform.eulerAngles.y;
                        else if ((diferenceRotation > 2.5 && canMoveRight)) eulerY = cameraTransform.eulerAngles.y;
