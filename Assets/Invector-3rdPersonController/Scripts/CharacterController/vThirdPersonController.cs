...
namespace Invector.CharacterController
{
    public class vThirdPersonController : vThirdPersonAnimator
    {
        ...
        void FixedUpdate()
        {
            ...
            CameraInput();                  // controls the camera using input X Y 
        }
        ...
        void AttackInput()
        {
            ...
            // check the input you select on the Action Input Painel 
            var _basicATKInput = actionsController.Attack.input.ToString();
            var _heavyATKInput = actionsController.HeavyAttack.input.ToString();
            ...
            if (inputType == InputType.Mobile)
            {
                if (CrossPlatformInputManager.GetButtonDown(_basicATKInput) && attackConditions)
                    animator.SetTrigger("MeleeAttack");
            }
            // keyboard & gamepad input
            else
            {
                if (Input.GetButtonDown(_basicATKInput) && attackConditions)
                    animator.SetTrigger("MeleeAttack");
                if (Input.GetAxis(_heavyATKInput) > 0f && attackConditions || Input.GetKeyDown(KeyCode.Alpha1) && attackConditions)
